...
export class PartnerApiService {
  ...
  refreshAllData(): void {
    ...
    this.getPartnerProfile().subscribe({
      ...
      error: (err) => console.error('Error loading partner profile:', err),
    });

    this.getPartnerStats().subscribe({
      next: (stats) => this.setKpiFromStats(stats),
      error: (err) => console.error('Error loading partner statistics:', err),
    });

    this.getRecentBookings().subscribe({
      next: (bookings) => this.setReservationsFromBookings(bookings),
      error: (err) => console.error('Error loading recent bookings:', err),
    });
  ...
  private setKpiFromStats(stats: any): void {
    ...
    const revenue = Number(
      source.revenueToday ?? source.revenusAujourdhui ?? source.netRevenue ?? 0,
    );
  ...
  private setReservationsFromBookings(bookings: any[]): void {
    const mapped: Reservation[] = (bookings ?? []).map((booking: any) => ({
      id: booking.id,
      reference: booking.reference || booking.bookingReference || `RES-${booking.id}`,
      passager:
        booking.passengerName || booking.passenger?.fullName || booking.passenger?.name || '—',
      trajet: booking.route || `${booking.departureCity || '—'} → ${booking.arrivalCity || '—'}`,
      date: booking.bookingDate || booking.createdAt || '',
      montant: Number(booking.price ?? booking.amount ?? booking.totalAmount ?? 0),
      statut: booking.status || booking.ticketStatus || booking.paymentStatus || 'en_attente',
    })) as Reservation[];
  ...
  getTripDetails(tripId: number | string): Observable<Trip> {
    return this.http.get<Trip>(`${this.apiUrl}/trips/${tripId}`).pipe(
      catchError((err) => {
        ...
        const found = this.trajets().find((t) => String(t.id) === String(tripId));
        if (found) {
  ...
  updateTrip(tripId: number | string, updates: Partial<Trip>): Observable<Trip> {
    return this.http.put<Trip>(`${this.apiUrl}/trips/${tripId}`, updates).pipe(
      tap((updated) => {
        this.trajets.update((list) =>
          list.map((t) => (String(t.id) === String(tripId) ? { ...t, ...updated } : t)),
        );
      }),
    );
  }

  deleteTrip(tripId: number | string): Observable<{ success: boolean; message: string }> {
    return this.http
      .delete<{
        success: boolean;
        message: string;
      ...
      .pipe(
        tap(() => {
          this.trajets.update((list) => list.filter((t) => String(t.id) !== String(tripId)));
        }),
      );
  }
  ...
  cancelTrip(
    ...
    return this.http
      ...
      .pipe(
        ...
        tap(() => {
          this.trajets.update((list) =>
            list.map((t) => (String(t.id) === String(tripId) ? { ...t, status: 'annule' } : t)),
          );
        }),
      );
  }
      ...
      map(
        (response) =>
          ({
            ...
            passengerName: response.passengerName || 'Passager Validé',
            boardingStatus: response.boardingStatus || 'VALID',
            message: response.message || 'Billet validé avec succès',
            boardingTime: response.boardingTime || new Date().toISOString(),
  ...
  getTripManifest(tripId: number | string): Observable<ManifestData> {
    ...
    return forkJoin({
      ...
      map(({ trip, tickets }) => {
        ...
        const pending = finalPassengers.filter((p) => p.boardingStatus === 'PENDING').length;
        const noShow = finalPassengers.filter((p) => p.boardingStatus === 'NO_SHOW').length;
        const cancelled = finalPassengers.filter((p) => p.boardingStatus === 'CANCELLED').length;
  ...
  updateAgency(agencyId: number | string, updates: Record<string, any>): Observable<any> {
    return this.http.put<any>(`${this.apiUrl}/agencies/${agencyId}/admin`, updates).pipe(
      tap(() => {
  ...
  /**
   * Propose un nouveau numéro mobile money pour les retraits de l'agence.
   * Ne modifie PAS payoutMsisdn directement : place le numéro en attente
   * de validation par un administrateur (voir AgencyController::proposePayoutMsisdn).
   */
  proposePayoutMsisdn(agencyId: number | string, payoutMsisdn: string): Observable<any> {
    return this.http
      .post<{
        message: string;
        pendingPayoutMsisdn: string;
      }>(`${this.apiUrl}/agencies/${agencyId}/payout-msisdn`, { payoutMsisdn })
      .pipe(
        tap((response) => {
          this.agence.update((curr) => ({
            ...curr,
            pendingPayoutMsisdn: response.pendingPayoutMsisdn,
            pendingPayoutMsisdnRequestedAt: new Date().toISOString(),
          }));
        }),
      );
  }
  ...
  updateAgencyProfile(updates: Partial<Agence>): Observable<Agence | null> {
    ...
    return this.http.patch<Agence>(`${this.apiUrl}/agencies/${agency.id}`, updates).pipe(
      tap((updatedAgency) => {
  ...
  getAnalyticsReport(period: string = '30j'): Observable<any> {
    ...
    const params = new HttpParams().set('from', from.toISOString()).set('to', now.toISOString());

    return forkJoin({
      analytics: this.http.get<any>(`${this.apiUrl}/partner/analytics`, {
        params,
      }),
      routes: this.http.get<any[]>(`${this.apiUrl}/partner/analytics/routes`, { params }),
    }).pipe(
      map(({ analytics, routes }) => {
        ...
        Object.entries(dailyNetSales).forEach(([date, amount]) => {
          ...
          monthlyMap.set(month, (monthlyMap.get(month) ?? 0) + Number(amount ?? 0));
        });
        ...
        return {
          ...analytics,
          totalRevenue: Number(analytics?.finance?.salesNetOfPlatformFee ?? 0),
          totalTrips,
  ...
  deleteAgencyDocument(documentId: number | string): Observable<{ success: boolean }> {
  ...
  getMaintenanceSchedule(): Observable<Bus[]> {
    return this.http.get<any>(`${this.apiUrl}/buses/maintenance-schedule`).pipe(
      unwrapCollection<Bus>(),
      catchError(() => of([])),
    );
  }
  ...
  getBusDetails(busId: number | string): Observable<Bus> {
    return this.http.get<Bus>(`${this.apiUrl}/buses/${busId}`).pipe(
      catchError((err) => {
        ...
        const found = this.bus().find((b) => String(b.id) === String(busId));
        if (found) {
          return of(found);
        }
        throw err;
  ...
  addBus(busData: Partial<Bus>): Observable<Bus> {
    const payload = {
      registrationNumber: busData.registrationNumber || busData.immatriculation,
  ...
  updateBus(busId: number | string, updates: Partial<Bus>): Observable<Bus> {
    return this.http.put<Bus>(`${this.apiUrl}/buses/${busId}`, updates).pipe(
      tap((updated) => {
        this.bus.update((list) =>
          list.map((b) => (String(b.id) === String(busId) ? { ...b, ...(updated as Bus) } : b)),
        );
      }),
      catchError((err) => {
  ...
  deleteBus(busId: number | string): Observable<{ success: boolean; message: string }> {
    return this.http
      .delete<{
        success: boolean;
        message: string;
      ...
      .pipe(
        tap(() => {
          this.bus.update((list) => list.filter((b) => String(b.id) !== String(busId)));
        }),
        catchError((err) => {
  ...
  getBusPoints(agencyId?: number | string): Observable<BusPoint[]> {
    ...
    return this.http.get<any>(`${this.apiUrl}/agency-points`, { params }).pipe(
  ...
  addBusPoint(pointData: Partial<BusPoint>): Observable<BusPoint> {
    return this.http.post<BusPoint>(`${this.apiUrl}/agency-points`, pointData).pipe(
      tap((newPt) => {
        this.pointsEmbarquement.update((list) => [newPt as PointEmbarquement, ...list]);
      }),
      catchError((err) => {
  ...
  getBusPointDetail(pointId: number | string): Observable<BusPoint> {
    return this.http.get<BusPoint>(`${this.apiUrl}/agency-points/${pointId}`).pipe(
      catchError((err) => {
        console.error('Error loading bus point details:', err);
        const found = this.pointsEmbarquement().find((p) => String(p.id) === String(pointId));
        if (found) {
  ...
  updateBusPoint(pointId: number | string, updates: Partial<BusPoint>): Observable<BusPoint> {
    return this.http.patch<BusPoint>(`${this.apiUrl}/agency-points/${pointId}`, updates).pipe(
      tap((updated) => {
  ...
  deleteBusPoint(pointId: number | string): Observable<{ success: boolean; message: string }> {
    return this.http
      .delete<{
        success: boolean;
        message: string;
  ...
  deletePoint(pointId: number | string): Observable<{ success: boolean; message: string }> {
  ...
  markNotificationAsRead(notificationId: number | string): Observable<{ success: boolean }> {
    return this.http
      ...
      .pipe(
        ...
        tap(() => {
          this.notifications.update((list) =>
            list.map((n) =>
              String(n.id) === String(notificationId) ? { ...n, lu: true, isRead: true } : n,
            ),
          );
        }),
        catchError(() => {
  ...
  markAllNotificationsAsRead(): Observable<{ success: boolean }> {
    return this.http.patch<any>(`${this.apiUrl}/user-notifications/mark-all-read`, {} as any).pipe(
      map(() => ({ success: true })),
      tap(() => {
        this.notifications.update((list) => list.map((n) => ({ ...n, lu: true, isRead: true })));
      }),
      catchError(() => {
        this.notifications.update((list) => list.map((n) => ({ ...n, lu: true, isRead: true })));
        return of({ success: true });
  ...
  deleteNotification(notificationId: number | string): Observable<{ success: boolean }> {
    return this.http
      .delete<{
        success: boolean;
      ...
      .pipe(
        tap(() => {
          this.notifications.update((list) =>
            list.filter((n) => String(n.id) !== String(notificationId)),
          );
        }),
        catchError(() => {
          this.notifications.update((list) =>
  ...
  createWithdrawal(withdrawal: {
    ...
    return this.http.post(`${this.apiUrl}/partner/withdrawals`, withdrawal).pipe(
      tap((newRetrait: any) => {
        const item: Retrait = {
          id: newRetrait?.id ?? `W00${this.retraits().length + 1}`,
          reference: newRetrait?.reference ?? `RET-${Date.now().toString().slice(-4)}`,
          montant: newRetrait?.amount ?? withdrawal.amount,
          demandeur: newRetrait?.demandeur ?? newRetrait?.requester ?? 'Utilisateur',
          dateDemande: newRetrait?.dateDemande ?? new Date().toISOString().slice(0, 10),
          statut: newRetrait?.statut ?? newRetrait?.status ?? 'en_attente',
        };
  ...
  cancelWithdrawal(withdrawalId: number | string): Observable<any> {
    return this.http.patch(`${this.apiUrl}/partner/withdrawals/${withdrawalId}/cancel`, {}).pipe(
      tap((cancelled) => {
  ...
  getWithdrawal(withdrawalId: number | string): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/partner/withdrawals/${withdrawalId}`).pipe(
      catchError((err) => {
        console.error('Error loading withdrawal details:', err);
        const found = this.retraits().find((r) => String(r.id) === String(withdrawalId));
        if (found) {
          return of(found);
        }
        throw err;
  ...
  getAllCities(): Observable<string[]> {
    return this.http.get<any>(`${this.apiUrl}/cities`).pipe(unwrapCollection<string>());
  }
  ...
  getStaffMembers(agencyId?: number | string): Observable<any[]> {
    ...
    return this.http.get<any>(`${this.apiUrl}/users/staff`, { params }).pipe(
      unwrapCollection<any>(),
      tap((st) => {
        this.staff.set(st ?? []);
      }),
    ...
    return this.http.post<any>(`${this.apiUrl}/users/staff`, payload).pipe(
      tap((newUser) => {
        const u: Utilisateur = {
          ...
          role: (newUser.agent?.agentRole || payload.agent?.agentRole || 'agent') as any,
          statut: newUser.status || newUser.statut || payload.agent?.status || 'actif',
          avatar: newUser.profilePhotoUrl || payload.fullName.slice(0, 2).toUpperCase(),
  ...
  loadRevenueChart(period: '7j' | '30j' | '12m' = '30j'): Observable<RevenueChartResponse> {
    ...
    const params = new HttpParams().set('start', startStr).set('end', endStr);
  ...
  getPointTypes(): Observable<SelectOption[]> {
    return this.http.get<SelectOption[]>(`${this.apiUrl}/bus-points/types`).pipe(
      catchError((err) => {
  ...
  getCityOptions(): Observable<SelectOption[]> {
  ...
  getPaymentMethods(): Observable<SelectOption[]> {
    return this.http.get<SelectOption[]>(`${this.apiUrl}/payment-methods`).pipe(
      catchError((err) => {
  ...
  getStatusOptions(): Observable<SelectOption[]> {
    return this.http.get<SelectOption[]>(`${this.apiUrl}/status-options`).pipe(
      catchError((err) => {
  ...
  getAgencyPointStatusOptions(): Observable<SelectOption[]> {
    return this.http.get<SelectOption[]>(`${this.apiUrl}/agency-points/status-options`).pipe(
      catchError((err) => {
        console.error('Error loading agency point status options:', err);
        throw err;
      }),
    );
  }
  ...
  getTripStatusOptions(): Observable<SelectOption[]> {
    return this.http.get<SelectOption[]>(`${this.apiUrl}/trips/status-options`).pipe(
      catchError((err) => {
  ...
  getTransactionTypeOptions(): Observable<SelectOption[]> {
    return this.http.get<SelectOption[]>(`${this.apiUrl}/transaction-types`).pipe(
      catchError((err) => {
        console.error('Error loading transaction type options:', err);
        throw err;
      }),
    );
  }
  ...
  getManifestStatusOptions(): Observable<SelectOption[]> {
    return this.http.get<SelectOption[]>(`${this.apiUrl}/manifests/status-options`).pipe(
      catchError((err) => {
        console.error('Error loading manifest status options:', err);
        throw err;
      }),
    );
  }
  ...
  deleteUser(userId: number | string): Observable<any> {
    return this.http.delete<any>(`${this.apiUrl}/users/staff/${userId}`).pipe(
      tap(() => {
        this.staff.update((list) => list.filter((u) => String(u.id) !== String(userId)));
      }),
      catchError((err) => {
